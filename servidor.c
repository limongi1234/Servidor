  ...
  struct Filmes 
  {
  	...
  	struct Cliente
	{
      ...
      int finalizar;
  	  ...
  	  filme[0].Status = "Disponível";
  	  filme[0].n_locacoes=0;
  
      filme[1].Filme = "Hercules";
  	  filme[1].Status = "Disponível";
  	  filme[1].n_locacoes = 0;

      filme[2].Filme = "Pânico";
  	  filme[2].Status = "Disponível";
	  ...
	  if(err != 0)
	  {
	     printf("Não consegui carregar a DLL WINSOCK.DLL.");
         return 0;
	  }
	...
	{
	    printf("Não consegui efetuar o bind.");
       return 0;
    }
    ...
    while(  !finalizar  )
    {
	   ...
	   printf("Aguardando o código do filme!\n");
